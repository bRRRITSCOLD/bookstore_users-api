...
func (user *User) GetByUserID() errors_utils.APIError {
	...
	if prepareErr != nil {
		...
		return errors_utils.NewInternalServerAPIError("internal server error", prepareErr)
	}
	...
	if scanStructErr := queryRowResult.StructScan(&foundUser); scanStructErr != nil {
		...
		return errors_utils.NewInternalServerAPIError("internal server error", scanStructErr)
	}
...
func (user *User) Save() errors_utils.APIError {
	...
	if insertErr != nil {
		...
		return errors_utils.NewInternalServerAPIError("internal server error", insertErr)
	}
	...
	if lastInsertedIDErr != nil {
		...
		return errors_utils.NewInternalServerAPIError("internal server error", lastInsertedIDErr)
	}
...
func (user *User) PutByUserID() errors_utils.APIError {
	...
	if putErr != nil {
		...
		return errors_utils.NewInternalServerAPIError("internal server error", putErr)
	}
...
func (user *User) DeleteByUserID() errors_utils.APIError {
	...
	if _, deleteErr := stmt.Exec(user.UserID); deleteErr != nil {
		...
		return errors_utils.NewInternalServerAPIError("internal server error", deleteErr)
	}
...
func (user *User) GetUsersByStatus(status string) (Users, errors_utils.APIError) {
	...
	if prepareErr != nil {
		...
		return nil, errors_utils.NewInternalServerAPIError("internal server error", prepareErr)
	}
	...
	if queryxErr != nil {
		...
		return nil, errors_utils.NewInternalServerAPIError("internal server error", queryxErr)
	}
	...
	for queryxRows.Next() {
		...
		if scanStructErr := queryxRows.StructScan(&foundUser); scanStructErr != nil {
			...
			return nil, errors_utils.NewInternalServerAPIError("internal server error", scanStructErr)
		}
	...
	if len(foundUsers) == 0 {
		return nil, errors_utils.NewNotFoundAPIError(fmt.Sprintf("no users found matching status %s", status), errors.New("database error"))
	}
